...
Механизм подключения компонентов с учетом их зависимостей реализуется функциями: `core_use_api()` и `core_use_handler()`. Эти функции являются самыми важными в работе системы Coffium, поэтому рассмотрим их детально. Обе функции используют функцию нормализации пути `route_std_path()`, которая:
