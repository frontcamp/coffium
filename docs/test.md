...
Подход минималистичный: без внешних зависимостей, конфигурации и отдельного раннера — только библиотека `/libs/inc.test.php` и вызов `core_launch_tests()` откуда удобно.
...
Глобальный запуск тестов: `core_launch_tests($path)`
====================================================
...
1) `$path` — каталог
    ...
    - для каждого элемента вызывается `core_launch_tests()` (рекурсия);
...
Для корректной работы `core_launch_tests()` нужно соблюдать простые правила:
...
- имя тестовой функции:
    ...
    - все вспомогательные функции в тестовых файлах рекомендуется:
        ...
        - либо выносить в отдельные файлы и подключать до вызова core_launch_tests().
...
- Для компонентов использовать однотипную структуру, например:
    ...
    - и отдельный «агрегатор» для их запуска через `core_launch_tests()`.
