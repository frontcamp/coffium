...
- если `$value === true`:
    - при наличии в запросе параметра verbose (`GET/POST`) на экран выводится строка `Assertion passed! ...`;
    - в лог ничего не пишется;
- если `$value !== true`:
    - в системный лог (`error_log`) пишется сообщение вида `Assertion failed! <func>(<args>) in <file>:<line>;`
...
- `core_assert_equal()` / `core_assert_defined_and_equal()` — мягкое сравнение;
- `core_assert_same()` / `core_assert_defined_and_same()` — жёсткое сравнение.
...
```php
core_assert_defined($c);                # проверка наличия константы
core_assert_defined_and_equal($c, $v);  # проверка наличия и мягкое равенство
core_assert_defined_and_same($c, $v);   # проверка наличия и жесткое равенство
...
2) `$path` — файл
    ...
    - проверка существования файла, иначе trigger_error() с E_USER_ERROR и die();
...
При наличии параметра verbose в запросе перед запуском каждого теста выводится HTML-заголовок вида:

`<hr>Test: file <b>...</b>, function <b>...</b><br>`
...
- запрос `?core-test` запустит все тесты, выводя только ошибки;
- запрос `?core-test&verbose=1` дополнительно покажет успешные ассерты и имена тестов.
